...
import { calcSelfmailerPrice } from "@/components/SelfmailerKonfigurator";
...
export default function Page() {
  const familie = useMemo(() => getSelfmailerFamilie("inata"), []);
  const name = familie?.name ?? "INATA";
  const beschreibung = familie?.beschreibung;
  const varianten = useMemo(() => familie?.varianten ?? [], [familie]);
  ...
  const nachUmfang = useMemo(
    ...
    [varianten, cfg.umfang]
  );
  const grammaturen = useMemo(
    () => unique(nachUmfang.map((v) => v.inhalt?.grammatur)),
    [nachUmfang]
  );
  const grammatur = cfg.grammatur && grammaturen.includes(cfg.grammatur) ? cfg.grammatur : null;

  const ausgewaehlteVariante: Produkt | undefined = nachUmfang.find((v) => v.inhalt?.grammatur === grammatur);

  const steps = useMemo(
    () => ["Auflage", "Umfang", "Grammatur", "Übersicht"] as const,
    []
  );
  ...
  const preisdetails = calcSelfmailerPrice("inata", cfg);
                    ...
                    {preisdetails ? (
                      <div className="border border-[#dcdcdc]">
                        ...
                        <div className="px-4 py-3 text-xs text-[#888888] border-t border-[#f0f0f0] leading-relaxed space-y-2">
                          <p>
                            Sendungen <Link href="/hilfe#dialogpost" className="text-[#822660] underline">müssen werblichen Inhalt</Link> enthalten, ansonsten kann es zu Preisaufschlägen durch die Deutsche Post AG kommen.
                          </p>
                          ...
                          <p><Link href="/hilfe#portooptimierung" className="text-[#822660] underline">Infos zur Portooptimierung</Link></p>
                        </div>
                      </div>
                    ) : null}
                  </div>
